   ...
   //if it is true put string =0 
	//	string operator +(string m[o]);
	   ...
	   virtual  BigInt operator+(const  BigInt& arg)
	    { BigInt argy=arg;
	     int min = size(), max = min, carry = 0, diff;
         bool whMax = true;
	     char nextDigit;
	     string res = "";
       ...
       {
       	   BigInt argy=arg;
       	   int min = size(), max = min, Borrow = 0, diff;
           bool whMax = true;
	       char nextDigit;
	       string result = "";
     	// Check which integer is longer		
	   ...
	   /* std::ostream& operator<<(ostream& out,const BigInt & arg
		)
           {
           	 BigInt argy=arg;
              out<<"Number :"<<argy.name<<endl;
              return out;
            }
          
            
            istream& operator>>(istream& in,BigInt& arg)
          {
           in>>arg.num;
            return in;
          }
